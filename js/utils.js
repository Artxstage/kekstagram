const DELAY = 500, getRandomInteger = (min, max) => {
  if (min > max) {
    [min, max] = [max, min];
  }
  if (min < 0) {
...
}, getNonExistenObject = (existingObjects, min, max) => {
  ...
  return id;
}, keyIsEscape = (evt) => evt.key === 'Escape', debounce = (callback, timeoutDelay = DELAY) => {
  let timeoutId;
  return (...rest) => {
